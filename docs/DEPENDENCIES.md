...
- Letter library from CAS (`docs/correspondence-templates/`) is now wired. Solicitor sign-off still required before live sending. Letter before action is still not a sendable pack.
...
- Document generation from CAS letter/email templates (`docs/correspondence-templates/`). Wording is in the CRM; solicitor sign-off is still required before live use, and the office mailbox is still not connected.
