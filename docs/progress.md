...
- Hire agreement renewal is a shared chase type. It counts from the current signed agreement start (day 80 alert, day 88 overdue), not from fleet booking dates, and not for courtesy/staff use unless a hire agreement is on the file. Logging a renewal adds a new agreement period and keeps the earlier ones. An ended hire never alerts. The supplied pack still says 89 days; the CRM operational limit stays 88 unless changed in Settings.
